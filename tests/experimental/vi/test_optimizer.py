...
class FakeDistribution:
    ...
    def __init__(self, *, loc, scale=None, covariance_matrix=None, **kwargs):
        ...
        self.covariance_matrix = None if covariance_matrix is None else jnp.asarray(covariance_matrix)
        ...
        self._vec = (
            ...
            if self._shape else jnp.array(0.0, dtype=jnp.float32)
        )
    ...
    def _make():
        return {
            "z": {
                ...
                "variational_param_bijectors": {"loc": FakeBijector(), "scale": FakeBijector()},
                "split_indices": [],
            }
        }
    return _make
...
def mock_model_interface():
    ...
    obs = Mock(); obs.observed = True; obs.value = Mock(); obs.value.shape = (4,)
    unobs = Mock(); unobs.observed = False; unobs.value = Mock(); unobs.value.shape = (2,)

    model = Mock(); model.vars = {"y_obs": obs, "u": unobs}
...
def disable_jit_and_zero_grads(monkeypatch):
    import jax
    monkeypatch.setattr(jax, "jit", lambda f=None, **kw: (lambda g: g) if f is None else f)
...
def test_accepts_tfp_distribution_class(make_latent_config):
    ...
    opt = Optimizer(seed=0, n_epochs=1, S=2, model_interface=DummyInterface({"z": jnp.array(0.0)}), latent_variables=latent)
...
def test_rejects_distribution_instance_not_class(make_latent_config):
    ...
    with pytest.raises(ValueError, match="dist_class.*class|TFP|Distribution"):
        Optimizer(seed=0, n_epochs=1, S=2, model_interface=DummyInterface({"z": jnp.array(0.0)}), latent_variables=latent)
...
def test_init_variational_params_missing_bijector_key_raises(make_latent_config):
    ...
    with pytest.raises(KeyError):
        _ = Optimizer(seed=0, n_epochs=1, S=2, model_interface=DummyInterface({"z": jnp.array(0.0)}), latent_variables=latent)
...
def test_init_variational_params_inverse_bijectors_scale_softplus_roundtrip(make_latent_config):
...
def test_init_optimizer_assigns_different_transforms_per_block():
    ...
    grads = {"z": {"loc": jnp.array(1.0), "scale": jnp.array(1.0)},
...
def test_init_structure_key_coherence_only():
    ...
    opt = Optimizer(seed=0, n_epochs=1, S=2, model_interface=DummyInterface(params), latent_variables=latent)
...
def test_build_variational_distribution_does_not_mutate_inputs(make_latent_config):
    latent = make_latent_config()
    opt = Optimizer(0, 1, 2, DummyInterface({"z": 0.0}), latent)
    vp_before = {k: v.copy() if hasattr(v, "copy") else v for k, v in opt.variational_params["z"].items()}
...
class TestSampleAndLogProbCompute:
    def test_mean_field_sampling_and_logprob(
        ...
        opt = Optimizer(seed=0, n_epochs=1, S=1, model_interface=mock_model_interface,
                        latent_variables=mean_field_cfg, batch_size=None, patience_tol=None, window_size=None)
    ...
    def test_composite_sampling_and_logprob(
        ...
        mock_model_interface.get_params.return_value = {"a": jnp.zeros((2,)), "b": jnp.zeros((1,))}
        opt = Optimizer(seed=0, n_epochs=1, S=1, model_interface=mock_model_interface,
                        latent_variables=composite_cfg, batch_size=None, patience_tol=None, window_size=None)
        rng = jax.random.PRNGKey(0)
        samples, total_log_q = opt._sample_and_compute_variational_log_prob(
            opt.variational_params, rng
        )
...
class TestElboComputation:
    def test_elbo(
        self, latent_cfg, mock_model_interface, patch_init_optimizer, monkeypatch,
    ):
        opt = Optimizer(seed=0, n_epochs=1, S=2, model_interface=mock_model_interface,
                        latent_variables=latent_cfg, batch_size=None, patience_tol=None, window_size=None)
        
        def fake_sample_and_logq(params, key):
            samples = {"a": jnp.array([0.0, 1.0], dtype=jnp.float32),
                       "b": jnp.array([0.0], dtype=jnp.float32)}
            total_log_q = jnp.array(0.5, dtype=jnp.float32)
            return samples, total_log_q

        monkeypatch.setattr(opt, "_sample_and_compute_variational_log_prob", fake_sample_and_logq)
        ...
        loss, _ = opt._elbo(opt.variational_params, rng, dim_data, batch_size, batch_indices, opt.S)
...
class TestGetFinalDistributions:
    def test_mapping_includes_each_name_and_composite_for_multivariate(
        self, mock_model_interface, composite_cfg, patch_init_optimizer, monkeypatch,
    ):
        opt = Optimizer(seed=0, n_epochs=1, S=1, model_interface=mock_model_interface,
                        latent_variables=composite_cfg, batch_size=None, patience_tol=None, window_size=None)
    ...
    def test_mapping_for_mean_field_separate_distributions(
        self, mock_model_interface, mean_field_cfg, patch_init_optimizer, monkeypatch,
    ):
        opt = Optimizer(seed=0, n_epochs=1, S=1, model_interface=mock_model_interface,
                        latent_variables=mean_field_cfg, batch_size=None, patience_tol=None, window_size=None)
...
class TestFitLoop:
    def test_fit_runs_with_noop_optimizer_and_records_elbo_and_final_dists(
        self, latent_cfg, mock_model_interface, patch_init_optimizer, monkeypatch,
        disable_jit_and_zero_grads, lax_loops_as_python, optax_noop_apply_updates,
    ):
        opt = Optimizer(seed=0, n_epochs=2, S=1, model_interface=mock_model_interface,
                        latent_variables=latent_cfg, batch_size=None, patience_tol=0.0, window_size=2)
