...
def test_default_build_engine_uses_opinionated_defaults():
    ...
    engine = LieselVI(model, loss_monitor=LOSS_MONITOR, seed=1).build_engine()
...
def test_old_batch_axis_size_shortcut_still_works():
    ...
    engine = LieselVI(
        model, loss_monitor=LOSS_MONITOR, batch_axis_size=2, seed=1
    ).build_engine()
...
def test_validation_monitor_raises():
    ...
    with pytest.raises(ValueError, match="does not support.*validation"):
        LieselVI(model, loss_monitor="validation")
...
def test_multi_size_default_split_builds_batch_manager():
    ...
    engine = LieselVI(
        model, loss_monitor=LOSS_MONITOR, batch_size=None, seed=1
    ).build_engine()
...
def test_scale_loss_false_builds_unscaled_default_loss():
    ...
    engine = LieselVI(
        model, loss_monitor=LOSS_MONITOR, scale_loss=False, seed=1
    ).build_engine()
...
def test_custom_loss_and_conflicting_split_raise():
    ...
    with pytest.raises(ValueError, match="loss.split"):
        LieselVI(model, loss_monitor=LOSS_MONITOR, loss=loss, split=other_split)
