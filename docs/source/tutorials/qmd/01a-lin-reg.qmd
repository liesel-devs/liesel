...
More on that in the [Model building with Liesel tutorial](01b-model.md#model-building)
...
and using a NUTS sampler for $\boldsymbol{\beta}$. More on sampling $\sigma^2$ can be found in the [Parameter transformations tutorial](01c-transform.md#parameter-transformation) and the
[Gibbs sampling tutorial](01d-gibbs-sampling.md#gibbs-sampling).
...
The engine can be configured step by step with a {class}`.EngineBuilder`. Starting from
a Liesel model, it is straight-forward to obtain an engine builder using the
...
returns a fully configured MCMC engine.
