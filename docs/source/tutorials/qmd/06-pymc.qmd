...
where $\nu$ is an hyper-parameter initialized to a small value.

First, we generate the data. We use a model with four intercepts but assume that only two variables are relvant, namely the first and the third one.
...
Since $\delta_j$ is a discrete variable, we need to use a Gibbs sampler for it. The {class}`.GibbsKernel` requires a function to perform the MCMC transition.
...
def delta_transition_fn(prng_key, model_state):
    ...
    proposal = {
        ...
        jax.random.bernoulli(draw_key, p=p, shape=(4,)),
        dtype=np.int64
        )
    }
...
Finally, we can sample from the posterior as we do for any other Liesel model. In this case, we use a {class}`.NUTSKernel` both for each model parameter and a {class}`.GibbsKernel` for $\boldsymbol{\delta}$.
...
As we can see from the values of the $\boldsymbol{\delta}$, the model was able to recognize those variable with no influence on the respose $\mathbf{y}$.
