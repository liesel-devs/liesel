...
The class {class}`.PyMCInterface` offers an interface between PyMC and Goose. By default, the constructor of {class}`.PyMCInterface` keeps track only of transformed random variables. To make the untransformed ones trackable for the Goose {class}`.Engine`, we must specify them in the constructor.
