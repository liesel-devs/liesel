...
In Bayesian P-Splines, we are working with a rank-deficient (degenerate)
multivariate normal prior on a vector of spline coefficients
...
In this last step, we create overarching group that incorporates the observed
basis matrix of our spline and uses the parameter groups to evaluate the smooth
...
3. In our `super().__init__` call at the end, we make sure to add not only the
