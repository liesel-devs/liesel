...
In Bayesian P-Splines, we are working with a rank-deficient (degenerate) multivariate
...
p(\boldsymbol{\beta}) \propto \exp\left(
    ...
    \right),
$$
where $\boldsymbol{K}$ is an appropriately defined penalty matrix, commonly a rank-deficient squared second order difference matrix where the variance and tje parameter $\tau^2$ controls the amount of strength of the penalty (usually a large $\tau^2$ allows for a wiggly function) and corresponds to an inverse smoothing parameter.
