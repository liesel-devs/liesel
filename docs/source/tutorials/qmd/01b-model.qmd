...
it is a deterministic function of some other nodes.
An exp-transformation mapping a
...
So let us consider the same model and data from the [linear regression tutorial](01a-lin-reg.md), where we had
the underlying model $y_i \sim \mathcal{N}(\beta_0 + \beta_1 x_i, \;\sigma^2)$ with the true parameters $\boldsymbol{\beta} = (\beta_0, \beta_1)' = (1, 2)'$ and $\sigma = 1$.
...
Note that you could also provide a {class}`.Var` instance for the
...
beta = lsl.Var.new_param(
    value=np.array([0.0, 0.0]), distribution=beta_prior, name="beta"
)
```
...
Since all other nodes are directly or indirectly connected to this node, the Model has
collected those nodes automatically.

To visualize the statistical graph of a model we call the {func}`.plot_vars()` function.
