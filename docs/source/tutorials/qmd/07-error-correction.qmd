...
assumed to be suspect of a measurement error.
...
is assigned to $x_i$. Additional flexibility is achieved by adding hyperpriors with
...
$\mathbf{b}_i'$ is defined as the vector of the evaluations of the basis function and $\boldsymbol{\beta}$ represents the corresponding vector of the basis coefficients. In our implementation we set $\beta_0 = 0$. We generate equidistant knots based on the observed $x$-values and construct cubic B-spline basis matrices for both functions using the Liesel {func}`.basis_matrix()` implementation. The number of knots is set to 20.
...
To enforce smoothness in the spline models, second-order
difference penalties in the prior precision matrices are implemented
...
```
Then we can implement functions to calculate the the standard deviation and mean of our response, using
{class}`.new_calc`.
...
To draw from $\mu_x$, $\tau^2_x$, $\tau^2_{\mu}$ and $\tau^2_{\sigma}$ we use Gibbs kernels ({class}`~.goose.GibbsKernel`) kernels, as this allows us to use custom transition functions for our parameters. The full conditionals are:
...
transition function We set up our engine and draw 5000 posterior samples.
...
gs.plot_trace(results, "coef_scale", range(3))
```

```{python}
# Plot the treace of all location coefficients
gs.plot_trace(results, "coef_mu", range(3))
```
