Work with distribution factors
==============================
...
Keep the observation axis meaningful
------------------------------------
...
.. code-block:: python
   ...
   components = lsl.Var.new_obs(
       values, lsl.Dist(tfd.Normal, loc=jnp.zeros(2), scale=1.0), name="components"
   ).update()
   vectors = lsl.Var.new_obs(
       values,
       lsl.Dist(tfd.MultivariateNormalDiag, loc=jnp.zeros(2), scale_diag=jnp.ones(2)),
       name="vectors",
   ).update()
   print(components.log_prob.shape, vectors.log_prob.shape)
...
.. code-block:: python
   ...
   mu = lsl.Var.new_param(
       0.0, lsl.Dist(tfd.Normal, loc=0.0, scale=2.5), name="mu"
   )
   ...
   response = lsl.Var.new_obs(
       ...
       lsl.Dist(Laplace, loc=mu, scale=scale),
       name="response",
   )
   custom_model = lsl.Model(response)
   print(float(custom_model.log_lik))
   scale.value = 2.0
   print(float(custom_model.log_lik))
...
Attach a density to a calculated quantity
-----------------------------------------
...
.. code-block:: python
   ...
   margin_a = lsl.Var.new_obs(
       jnp.array([-0.5, 0.2, 0.8]), lsl.Dist(tfd.Normal, loc=0.0, scale=1.0), name="a"
   )
   margin_b = lsl.Var.new_obs(
       jnp.array([-0.2, 0.3, 0.4]), lsl.Dist(tfd.Normal, loc=0.0, scale=1.0), name="b"
   )
   pit_a = lsl.PIT(margin_a, name="pit_a").update()
   pit_b = lsl.PIT(margin_b, name="pit_b").update()
   copula = lsl.Var.new_calc(
       lambda a, b: jnp.stack([a, b], axis=-1), pit_a, pit_b,
       dist=lsl.Dist(GaussianCopula, dependence=0.3), name="copula",
   )
   copula.observed = True
   copula_model = lsl.Model(copula)
   print(float(copula_model.log_lik))
...
:doc:`model-transformations` for changes of parameter coordinates.
...
.. code-block:: python
   ...
   print(pointwise[response_key].shape)
