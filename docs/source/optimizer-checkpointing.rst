...
With the built-in :class:`.NegLogProbLoss` or :class:`.NegElboLoss` and optimizers,
anonymous internal nodes and constants do not need stable names: their read-only
evaluation state is rebuilt from the caller's model.
