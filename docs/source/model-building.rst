...
.. code-block:: python
   ...
   y = lsl.Var.new_obs(
       ...
       lsl.Dist(tfd.Normal, loc=mu, scale=1.0),
       name="y",
   )
   model = lsl.Model(y)
   print(float(model.log_prob))

``Model(y)`` collects the response and its inputs. The printed value is the
...
.. toctree::
   ...
   tutorials/notebooks/11-model-building
   tutorials/notebooks/12-model-predictions
