Combine and reuse model components
==================================
...
.. code-block:: python
   ...
   mean_a = lsl.Var.new_param(
       0.0, lsl.Dist(tfd.Normal, loc=0.0, scale=2.0), name="mean"
   )
   y_a = lsl.Var.new_obs(
       jnp.array([0.8, 1.2]), lsl.Dist(tfd.Normal, loc=mean_a, scale=1.0), name="y_a"
   )
   ...
   y_b = lsl.Var.new_obs(
       jnp.array([1.4, 1.6]), lsl.Dist(tfd.Normal, loc=mean_b, scale=0.5), name="y_b"
   )
   ...
   print(list(separate.parameters))
...
Share a parameter across responses
----------------------------------
...
.. code-block:: python
   ...
   print(list(shared.parameters))
   print(shared.vars["y_b"].dist_node["loc"] is shared.vars["mean"])
   shared.plot(width=7, height=5)

There is one ``mean`` parameter, and the identity check prints ``True``. Both
